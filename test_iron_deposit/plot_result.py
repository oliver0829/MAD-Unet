# -------------------------------------------------------------
# File: test_iron_deposit\plot_result.py
# Author: Qiang Li
# Date of Completion: June 27, 2024
# Description: plot result for Figure 14
# -------------------------------------------------------------
...
for index, batch in enumerate(data_loader):
    ...
    output_binary = (output > 0.9).astype(np.int8)


    # get TMI
    tmi_data = pd.read_csv(f'./csv/case{index}/deltaT.csv', sep=',', header=None).to_numpy()
    tmi = tmi_data[:, 2].reshape((341, 484))
    tmi = tmi[:, 71:412]
    tmi = interpolate(tmi, 100, 100)
    tmi[np.isnan(tmi)] = 0
    input = np.load(f'./data/case{index}.npy')
    ...
    cax = divider.append_axes("right", size="5%", pad=0.05)
    cbar = plt.colorbar(im12, cax=cax)
    cbar.ax.tick_params(labelsize=12)
